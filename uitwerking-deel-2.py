...
def play_guess_the_region_game(countries):
    ...
    for country in countries:
        if 'region' in country and 'name' in country:
    ...
    if os.path.exists(SCORE_FILE):
        with open(SCORE_FILE, "r") as file:
            ...
            for line in lines:
                ...
                score_entry = stripped_line.split(",")
...
def main():
    ...
    username = input("Voer je gebruikersnaam in: ").strip()

    # Haal landeninformatie op
    countries = get_countries_data()
    if countries:
        # Start het spel en krijg de score terug
        score = play_guess_the_region_game(countries)
        # Sla de score op
